...
@export_node_path("Node2D") var layer_to_hide_path
...
func _on_body_entered(body: Node):
	...
	var layer_to_hide = get_node_or_null(layer_to_hide_path)
	if interior == null or walls_front == null or layer_to_hide == null:
	...
	if is_between:
		...
		print("🚪 Player a intrat în casă")
	
	if layer_to_hide:
		layer_to_hide.visible = false
		print("👻 Layer ascuns: ", layer_to_hide.name)
